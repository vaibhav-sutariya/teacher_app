...
class AddHolidayHwBloc extends Bloc<AddHolidayHwEvent, AddHolidayHwState> {
  ...
  AddHolidayHwBloc({required this.repository}) : super(AddHolidayHwState.initial()) {
    on<UpdateHolidayHwClassEvent>((event, emit) => emit(state.copyWith(selectedClass: event.selectedClass)));
    on<UpdateHolidayHwSectionEvent>((event, emit) => emit(state.copyWith(section: event.section)));
    on<UpdateHolidayHwSubjectEvent>((event, emit) => emit(state.copyWith(subject: event.subject)));
    on<UpdateHolidayHwNameEvent>((event, emit) => emit(state.copyWith(hwName: event.hwName)));
    on<UpdateHolidayHwPdfUrlEvent>((event, emit) => emit(state.copyWith(pdfUrl: event.pdfUrl)));
