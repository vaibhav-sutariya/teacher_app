        ...
        listener: (context, state) {
          if (state.submitSuccess) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Holiday Homework added successfully'),
              ),
            );
            context.router.back();
    ...
    return BlocSelector<AddHolidayHwBloc, AddHolidayHwState, String?>(
      ...
      builder: (context, value) {
        return AppDropdownField<String>(
          ...
          itemLabel: (item) => item,
          onChanged: (val) => context.read<AddHolidayHwBloc>().add(
            UpdateHolidayHwClassEvent(val!),
          ),
        );
      },
    );
    ...
    return BlocSelector<AddHolidayHwBloc, AddHolidayHwState, String?>(
      ...
      builder: (context, value) {
        return AppDropdownField<String>(
          ...
          itemLabel: (item) => item,
          onChanged: (val) => context.read<AddHolidayHwBloc>().add(
            UpdateHolidayHwSectionEvent(val!),
          ),
        );
      },
    );
    ...
    return BlocSelector<AddHolidayHwBloc, AddHolidayHwState, String?>(
      ...
      builder: (context, value) {
        return AppDropdownField<String>(
          ...
          itemLabel: (item) => item,
          onChanged: (val) => context.read<AddHolidayHwBloc>().add(
            UpdateHolidayHwSubjectEvent(val!),
          ),
        );
      },
    );
        ...
        return Column(
          ...
          children: [
            AppDashedUploadButton(
              ...
              label: isUploaded
                  ? 'Holiday HW Uploaded'
                  : 'Upload Holiday HW PDF*',
              onTap: () {
                context.read<AddHolidayHwBloc>().add(
                  const UpdateHolidayHwPdfUrlEvent(
            ...
            child: BlocSelector<AddHolidayHwBloc, AddHolidayHwState, bool>(
              ...
              builder: (context, isLoading) {
                return AppPrimaryButton(
                  text: 'SAVE WORK',
                  isLoading: isLoading,
                  onPressed: () => context.read<AddHolidayHwBloc>().add(
                    SubmitHolidayHwEvent(),
                  ),
                );
              },
            ),
