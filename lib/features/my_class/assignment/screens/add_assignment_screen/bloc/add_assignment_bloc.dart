...
class AddAssignmentBloc extends Bloc<AddAssignmentEvent, AddAssignmentState> {
  ...
  AddAssignmentBloc({required this.repository})
    : super(AddAssignmentState.initial()) {
    on<UpdateClassEvent>(
      (event, emit) => emit(state.copyWith(selectedClass: event.selectedClass)),
    );
    on<UpdateSectionEvent>(
      (event, emit) => emit(state.copyWith(section: event.section)),
    );
    on<UpdateSubjectEvent>(
      (event, emit) => emit(state.copyWith(subject: event.subject)),
    );
    on<UpdateLessonEvent>(
      (event, emit) => emit(state.copyWith(lesson: event.lesson)),
    );
    on<UpdateTopicEvent>(
      (event, emit) => emit(state.copyWith(topic: event.topic)),
    );
    on<UpdateAttachmentTypeEvent>(
      (event, emit) =>
          emit(state.copyWith(attachmentType: event.attachmentType)),
    );
    on<UpdateDescriptionEvent>(
      (event, emit) => emit(state.copyWith(description: event.description)),
    );
    on<ToggleSmsEvent>(
      (event, emit) => emit(state.copyWith(isSmsEnabled: event.isSmsEnabled)),
    );
