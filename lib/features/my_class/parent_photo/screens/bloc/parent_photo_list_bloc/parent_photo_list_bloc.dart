...
class ParentPhotoListBloc
    extends Bloc<ParentPhotoListEvent, ParentPhotoListState> {
  ...
  void _onLoadParents(
    LoadParents event,
    Emitter<ParentPhotoListState> emit,
  ) async {
    emit(
      state.copyWith(
        status: ParentPhotoListStatus.loading,
    ...
    try {
      ...
      emit(
        state.copyWith(
          status: ParentPhotoListStatus.error,
  ...
  void _onSearchParents(
    ...
    emit(state.copyWith(searchQuery: event.query, filteredParents: filtered));
  }

  void _onUpdateParentPhoto(
    UpdateParentPhoto event,
    Emitter<ParentPhotoListState> emit,
  ) {
    ...
    emit(state.copyWith(parents: updatedParents, filteredParents: filtered));
  }
}
