...
class AddExamTimeTableBloc extends Bloc<AddExamTimeTableEvent, AddExamTimeTableState> {
  final ExamTimeTableRepository repository;

  AddExamTimeTableBloc({required this.repository}) : super(AddExamTimeTableState.initial()) {
    on<UpdateExamTimeTableClassEvent>((event, emit) => emit(state.copyWith(selectedClass: event.selectedClass)));
    on<UpdateExamTimeTableNameEvent>((event, emit) => emit(state.copyWith(examName: event.examName)));
    on<UpdateExamTimeTablePdfUrlEvent>((event, emit) => emit(state.copyWith(pdfUrl: event.pdfUrl)));
  ...
  Future<void> _onSubmitExamTimeTable(
    ...
    if (state.selectedClass == null || state.examName == null || state.pdfUrl == null) {
