        ...
        listener: (context, state) {
          if (state.submitSuccess) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Exam Time Table added successfully'),
              ),
            );
            context.router.back();
    ...
    return BlocSelector<AddExamTimeTableBloc, AddExamTimeTableState, String?>(
      ...
      builder: (context, value) {
        return AppDropdownField<String>(
          ...
          itemLabel: (item) => item,
          onChanged: (val) => context.read<AddExamTimeTableBloc>().add(
            UpdateExamTimeTableClassEvent(val!),
          ),
        );
      },
    );
    ...
    return BlocSelector<AddExamTimeTableBloc, AddExamTimeTableState, String?>(
      ...
      builder: (context, value) {
        return AppDropdownField<String>(
          ...
          itemLabel: (item) => item,
          onChanged: (val) => context.read<AddExamTimeTableBloc>().add(
            UpdateExamTimeTableNameEvent(val!),
          ),
        );
      },
    );
        ...
        return Column(
          ...
          children: [
            AppDashedUploadButton(
              ...
              label: isUploaded
                  ? 'Time Table Uploaded'
                  : 'Upload Exam Time Table PDF*',
              onTap: () {
                context.read<AddExamTimeTableBloc>().add(
                  const UpdateExamTimeTablePdfUrlEvent(
    ...
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
            child:
                BlocSelector<AddExamTimeTableBloc, AddExamTimeTableState, bool>(
                  selector: (state) => state.isSubmitting,
                  builder: (context, isLoading) {
                    return AppPrimaryButton(
                      text: 'SAVE TABLE',
                      isLoading: isLoading,
                      onPressed: () => context.read<AddExamTimeTableBloc>().add(
                        SubmitExamTimeTableEvent(),
                      ),
                    );
                  },
                ),
