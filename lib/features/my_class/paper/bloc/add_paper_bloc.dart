...
class AddPaperBloc extends Bloc<AddPaperEvent, AddPaperState> {
  ...
  AddPaperBloc({required this.repository}) : super(AddPaperState.initial()) {
    on<UpdatePaperClassEvent>((event, emit) => emit(state.copyWith(selectedClass: event.selectedClass)));
    on<UpdatePaperSectionEvent>((event, emit) => emit(state.copyWith(section: event.section)));
    on<UpdatePaperSubjectEvent>((event, emit) => emit(state.copyWith(subject: event.subject)));
    on<UpdatePaperNameEvent>((event, emit) => emit(state.copyWith(paperName: event.paperName)));
    on<UpdatePaperPdfUrlEvent>((event, emit) => emit(state.copyWith(pdfUrl: event.pdfUrl)));
