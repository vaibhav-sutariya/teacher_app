...
class NewAdmissionBloc extends Bloc<NewAdmissionEvent, NewAdmissionState> {
  ...
  void _onLoadMonthAdmissions(
      LoadMonthAdmissions event, Emitter<NewAdmissionState> emit) async {
    ...
    try {
      ...
      final total = mockAdmissions.fold<int>(
          0, (sum, item) => sum + item.admissionCount);
      ...
      emit(state.copyWith(
          status: NewAdmissionStatus.error, errorMessage: e.toString()));
    }
  }

  void _onChangeMonth(
      ChangeAdmissionMonth event, Emitter<NewAdmissionState> emit) {
