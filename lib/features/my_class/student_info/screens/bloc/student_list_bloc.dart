...
class StudentListBloc extends Bloc<StudentListEvent, StudentListState> {
  ...
  StudentListBloc({required this.repository})
  ...
  void _onFilterStudents(
    ...
    if (event.query.isEmpty) {
      emit(
        state.copyWith(filteredStudents: state.allStudents, searchQuery: ''),
      );
    } else {
      final filtered = state.allStudents
          ...
          .toList();
      emit(
        state.copyWith(filteredStudents: filtered, searchQuery: event.query),
      );
    }
  }
}
