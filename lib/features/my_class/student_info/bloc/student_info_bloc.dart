...
class StudentInfoBloc extends Bloc<StudentInfoEvent, StudentInfoState> {
  ...
  StudentInfoBloc({required this.repository}) : super(StudentInfoState.initial()) {
    on<ChangeTabEvent>((event, emit) => emit(state.copyWith(tabIndex: event.index)));
  ...
  Future<void> _onSearchStudents(
    ...
    emit(state.copyWith(isLoading: true, searchQuery: event.query, error: null));
