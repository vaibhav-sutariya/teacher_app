...
class StaffAttendanceBloc
  ...
  Future<void> _onLoadAttendanceSummary(
    ...
    emit(
      state.copyWith(
        status: StaffAttendanceStatus.loading,
    ...
    try {
      ...
      emit(
        state.copyWith(status: StaffAttendanceStatus.loaded, summary: summary),
      );
    } catch (e) {
      emit(
        state.copyWith(
  ...
  Future<void> _onRefreshAttendanceData(
    ...
    if (state.status == StaffAttendanceStatus.loaded) {
      ...
      try {
        ...
        emit(state.copyWith(summary: summary, isRefreshing: false));
      } catch (e) {
