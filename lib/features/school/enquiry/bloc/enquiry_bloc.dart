...
class EnquiryBloc extends Bloc<EnquiryEvent, EnquiryState> {
  ...
  Future<void> _onLoadEnquiries(LoadEnquiries event, Emitter<EnquiryState> emit) async {
    ...
    try {
      ...
      emit(EnquiryLoaded(selectedMonth: state.selectedMonth, enquiries: enquiries));
    } catch (e) {
      emit(EnquiryError(selectedMonth: state.selectedMonth, message: e.toString()));
    }
  }
